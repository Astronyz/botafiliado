...
async function graphql(query, variables, cache) {
  const appId = cache?.getConfig('SHOPEE_APP_ID') ?? process.env.SHOPEE_APP_ID;
  const secret = cache?.getConfig('SHOPEE_APP_SECRET') ?? process.env.SHOPEE_APP_SECRET;
  if (!appId || !secret) throw new Error('Credenciais Shopee ausentes (SHOPEE_APP_ID/SHOPEE_APP_SECRET).');
  const payload = JSON.stringify({ query, variables });
  const body = await requestJson((cache?.getConfig('SHOPEE_API_URL') ?? process.env.SHOPEE_API_URL) || 'https://open-api.affiliate.shopee.com.br/graphql', {
...
export async function getShopeeProduct(url, cache) {
  ...
  const data = await graphql(query, { url }, cache);
